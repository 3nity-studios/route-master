...
City::City(int _id, std::string _name, Designar::Graph<BusStop, int> _city_map)
...
void City::add_street(int distance, int src_id, int tgt_id)
{   

    Designar::GraphNode<BusStop, int, Designar::EmptyClass> *src_stop = nullptr, *tgt_stop = nullptr;
    ...
    {
        city_map.insert_arc(src_stop, tgt_stop, distance);
    }
    else
    {
...
void City::run_simulation(Bus &bus, Employee &driver, int time, std::list<Designar::GraphArc<Designar::GraphNode<BusStop, int, Designar::EmptyClass>, BusStop, int, Designar::EmptyClass> *> path)
{
    ...
    for(auto track : path)
    {
        ...
        driver.calc_fatigue(track->get_info());
