#ifdef _WIN32
#include "platform/include/win32/windows_platform.hpp"
...
void WindowsPlatform::set_icon(const sf::WindowHandle &in_handle)
{
    ...
    if (smallIcon)
        SendMessage(in_handle, WM_set_icon, ICON_SMALL, (LPARAM)smallIcon);

    if (bigIcon)
        SendMessage(in_handle, WM_set_icon, ICON_BIG, (LPARAM)bigIcon);
