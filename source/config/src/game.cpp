...
void Game::init_window()
{
    ...
    float screen_scaling_factor = _platform.get_screen_scaling_factor(this->_data->window->getSystemHandle());

    // Use the screenScalingFactor to create video mode and set screen size - 720p by default
    this->_video_mode.height = SCREEN_HEIGHT * screen_scaling_factor;
    this->_video_mode.width = SCREEN_WIDTH * screen_scaling_factor;
    ...
    _platform.set_icon(this->_data->window->getSystemHandle());
