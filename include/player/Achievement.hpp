...
class Achievement
{
  private:
    ...
    std::vector<std::function<bool(Player &player, std::vector<SimulationInfo> &simulation_info)>>
        criteria;
    std::vector<std::function<int(Player &player, std::vector<SimulationInfo> &simulation_info)>>
        progress;
    std::vector<int> rewards;
    int current_level;
    int last_claimed_level;
    bool completed;
  ...
  public:
    ...
    std::vector<std::function<bool(Player &player, std::vector<SimulationInfo> &simulation_info)>> get_criteria() const;
    ...
    void set_criteria(const std::vector<std::function<bool(Player &player, std::vector<SimulationInfo> &simulation_info)>> &_criteria);
    void set_rewards(const std::vector<int> &_rewards);
    void set_current_level(const int &_current_level);

    void check_and_unlock(Player &player, std::vector<SimulationInfo> &simulation_info);
    int get_progress(Player &player, std::vector<SimulationInfo> &simulation_info);
    int get_accumulated_rewards(bool claim);
};