...
class MapLayer final : public sf::Drawable
{
  public:
    ...
    tmx::TileLayer::Tile getTile(std::int32_t tileX, std::int32_t tileY)
    {
        sf::Vector2u chunkLocale;
        const auto &selectedChunk = getChunkAndTransform(tileX, tileY, chunkLocale);
        return selectedChunk->getTile(static_cast<std::int32_t>(chunkLocale.x),
                                      static_cast<std::int32_t>(chunkLocale.y));
    }
    void setColor(std::int32_t tileX, std::int32_t tileY, sf::Color color, bool refresh = true)
    {
        sf::Vector2u chunkLocale;
        const auto &selectedChunk = getChunkAndTransform(tileX, tileY, chunkLocale);
        selectedChunk->setColor(static_cast<std::int32_t>(chunkLocale.x), static_cast<std::int32_t>(chunkLocale.y),
                                color, refresh);
    }
    ...
    sf::Color getColor(std::int32_t tileX, std::int32_t tileY)
    {
        sf::Vector2u chunkLocale;
        const auto &selectedChunk = getChunkAndTransform(tileX, tileY, chunkLocale);
        return selectedChunk->getColor(static_cast<std::int32_t>(chunkLocale.x),
                                       static_cast<std::int32_t>(chunkLocale.y));
    }
                    ...
                    for (auto x = xPos; static_cast<float>(x) < static_cast<float>(xPos) + chunkTileCount.x; ++x)
                    {
                            ...
                            m_chunkTileIDs[idx].ID <= ca->m_lastGID)
                        {
                            ...
                            doFlips(m_chunkTileIDs[idx].flipFlags, &tile[0].texCoords, &tile[1].texCoords,
                                    &tile[2].texCoords, &tile[3].texCoords, &tile[4].texCoords, &tile[5].texCoords);
                            ca->addTile(tile);
                        }
                        idx++;
                    }
        ...
        for (auto y = 0u; y < m_chunkCount.y; ++y)
        {
            ...
            for (auto x = 0u; x < m_chunkCount.x; ++x)
            {
                ...
                m_chunks.emplace_back(std::make_unique<Chunk>(
                    layer, usedTileSets,
                    sf::Vector2f(static_cast<float>(x) * m_chunkSize.x, static_cast<float>(y) * m_chunkSize.y),
                    tileCount, tileSize, map.getTileCount().x, m_textureResource, map.getAnimatedTiles()));
            }
        }
    }
