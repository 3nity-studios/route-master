...
class TrafficLight : public VisualElement
{
  private:
    std::vector<std::pair<StreetConnectionIDs, bool>>
        connections; // The second item of the pair is a bool that contains true if the light is green and false if the
                     // light is red
    int time_to_change;
    int last_change;
  ...
  public:
    TrafficLight();
    TrafficLight(int _id, std::vector<std::pair<StreetConnectionIDs, bool>> _connections, int _time_to_change, float _x,
                 float _y);
